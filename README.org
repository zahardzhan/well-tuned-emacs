...
пользователя. Однако, общий объем и широта этих улучшений со временем
превратили мощный текстовый редактор во всеобъемлющую
...
← [[elisp:(find-file (concat (file-name-as-directory (concat user-emacs-directory "custom")) "custom.el"))][~/.emacs.d/custom/custom.el]].
  ...
  (let ((custom-directory (file-name-as-directory (concat user-emacs-directory "custom"))))
    ...
    (add-hook 'emacs-startup-hook
      (defun save-custom-file-location-in-custom-file ()
        (unless (equal custom-file (cl-first (get 'custom-file 'saved-value)))
...
[[https://melpa.org][MELPA]][[[elisp:(progn (unless (featurep 'cl-lib) (require 'cl-lib))(unless (featurep 'package) (require 'package))(customize-save-variable 'package-archives (cl-pushnew (cons "melpa" "http://melpa.milkbox.net/packages/") package-archives :test #'equal)))][добавить]]]
и
...
пакетов гораздо больше, но они менее качественные.
...
80% опций главном в меню эмакса никогда не используются, остальные 20%
...
← [[elisp:(customize-save-variable 'uniquify-buffer-name-style 'forward)][имена одинаковых буферов предварены частью файлового пути]].

[[elisp:(customize-variable-other-window 'column-number-mode)][Column Number Mode]]
← [[elisp:(customize-save-variable 'column-number-mode t)][номер столбца текущей позиции курсора в моделайне]].
...
или
[[elisp:(customize-save-variable 'initial-buffer-choice t)][открывает =*scratch*=-буфер]] ---
вместо стартового экрана. Как вариант, эмакс может открыть любой файл,
директорию или все что угодно, на выбор.
  ...
  (add-hook 'emacs-startup-hook
    (defun well-tuned-emacs-scratch-buffer-message ()
      (with-current-buffer "*scratch*"
        (save-excursion
          (let ((initial-scratch-message-last-char (1- (point-max))))
            (goto-char initial-scratch-message-last-char)
            (fancy-splash-insert
             "To start: "
             :link `("open file"
             ...
             ".")
            (comment-region initial-scratch-message-last-char (point)))
          (when (buffer-modified-p)
            (set-buffer-modified-p nil))))))
#+end_src

[[elisp:(customize-variable-other-window 'blink-cursor-mode)][Blink Cursor Mode]]
← [[elisp:(customize-save-variable 'blink-cursor-mode t)][курсор мерцает]]. Так его легче найти.
...
Иначе попытка сдвинуть курсор в пустом буфере вызовет раздражительный
громкий звон.

[[elisp:(customize-variable-other-window 'save-place-mode)][Save Place Mode]]
← [[elisp:(customize-save-variable 'save-place-mode t)][позиция курсора сохраняется между сессиями]].
Курсор устанавливается на то место, где он был перед закрытием буфера.

[[elisp:(customize-variable-other-window 'save-place-file)][Save Place File]]
← [[elisp:(customize-save-variable 'save-place-file (concat user-emacs-directory "places"))][позиции курсора хранятся в файле ~/.emacs.d/places]].
...
aы] фыв фыв  ф
русский язык --- альтернативный метод ввода.
фывВ противном случае 
