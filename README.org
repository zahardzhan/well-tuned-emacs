...
открыть ее в эмаксе и выполнить команду [[elisp:org-babel-tangle][M-x org-babel-tangle]] ([[elisp:(describe-key (kbd "C-c C-v C-t"))][С-c C-v C-t]]).
...
| =~/.emacs= relative by default | =~/.emacs= relative by default                | =~/.emacs= relative         |
...
#+begin_src elisp
  (default "~/.emacs")
  (~/_emacs (expand-file-name "~/_emacs"))
  (~/_emacs.el (expand-file-name "~/_emacs.el"))
  (~/_emacs.elc (expand-file-name "~/_emacs.elc"))
  (~/.emacs (expand-file-name "~/.emacs"))
  (~/.emacs.el (expand-file-name "~/.emacs.el"))
  (~/.emacs.elc (expand-file-name "~/.emacs.elc"))
  (~/.emacs.d/init.el (expand-file-name "~/.emacs.d/init.el"))
  (~/.emacs.d/init.elc (expand-file-name "~/.emacs.d/init.elc"))
#+end_src

#+header: :noweb-ref customize-well-tuned-emacs
#+begin_src elisp
     (defvar actual-user-init-file)
     (setq actual-user-init-file
       (let (
...
Расположение файла этого руководства. Может быть символической ссылкой
(на файл в системе контроля версий). Для обеспечения переносимости
...
#+header: :noweb-ref customize-well-tuned-emacs
#+begin_src elisp
  <<define-well-tuned-emacs-reference-file-truename>>
  (defcustom well-tuned-emacs-reference-file
    (let* (
           <<user-init-file-names>>
           (~/readme "~/README.org")
           (~/.emacs.d/readme (locate-user-emacs-file "README.org")))
    ...
    :set (lambda (symbol value)
           ...
           <<update-well-tuned-emacs-reference-file-truename>>
           )
    :version "25.0.50.1"
...
Далее нужно будет делать различие между именем руководства как
возможной символической ссылкой на текстовый файл и реальным именем
текстового файла в котором хранится руководство. Получение реального
имени функцией file-truename во многие десятки раз медленнее обычных
файловых операций типа expand-file-name поэтому ее следует выполнять
как можно реже при изменении значения well-tuned-emacs-reference-file.

#+name: define-well-tuned-emacs-reference-file-truename
#+begin_src elisp
  (defvar well-tuned-emacs-reference-file-truename)
#+end_src

#+name: update-well-tuned-emacs-reference-file-truename
#+begin_src elisp
  (setq well-tuned-emacs-reference-file-truename
...
#+header: :noweb-ref customize-customize-and-apply-customizations
#+begin_src elisp
  (setq custom-file (locate-user-emacs-file "custom.el"))
  (load custom-file :noerror :nomessage)
  (unless (file-equal-p custom-file (car (get 'custom-file 'saved-value)))
    (add-hook 'after-init-hook
      (defun save-custom-file-location-in-custom-file ()
        (customize-save-variable 'custom-file custom-file))))
#+end_src
...
#+begin_src elisp
  (when (file-exists-p well-tuned-emacs-reference-file-truename)
    (or (not (file-exists-p actual-user-init-file))
        (file-newer-than-file-p well-tuned-emacs-reference-file-truename
                                actual-user-init-file)))
#+end_src
...
#+begin_src elisp
  (progn
    ...
    (let ((user-init-file actual-user-init-file))
      (org-babel-with-temp-filebuffer well-tuned-emacs-reference-file-truename
        (org-babel-tangle))
...
#+begin_src elisp
  <<dont-activate-packages-that-require-newer-emacs>>
  <<initialize-package-system>>
  <<make-sure-melpa-is-used-as-one-of-community-package-archives>>
  <<make-sure-gnu-elpa-keyring-is-up-to-date-for-old-emacs>>
...
После составления списка установленных пакетов
[[elisp:(describe-function 'package-initialize)][package-initialize]]
начинает активацию пакетов
[[elisp:(describe-function 'package-activate)][package-activate]],
[[elisp:(describe-function 'package-activate-1)][package-activate-1]] ---
предоставляет пользователю «автозагружаемые» функции определенные в
этих пакетах, но не загружает файлы в которых функции определены до
тех пор пока они не будут вызваны (см. автозагрузка:
[[info:Elisp#Autoload][инфо]],
[[https://www.gnu.org/software/emacs/manual/html_node/elisp/Autoload.html][мануал]],
[[https://emacsdocs.org/docs/elisp/Autoload][Emacs Docs]]).

В каждом пакете указано требование к минимальной версии эмакса. Во
время активации не производится проверка, сможет ли пакет работать на
текущей версии эмакса, что приводит к сбоям при активации или
выполнении кода пакета на версиях эмакса более ранних чем необходима.

Совет оборачивает процедуру активации и не активирует пакет если ему
требуется более новая версия эмакса. Это имеет смысл только если
установлено несколько версий эмакса которые используются с одним и тем
же комплектом пакетов и настроек (в основном для тестирования).

#+name: dont-activate-packages-that-require-newer-emacs
#+begin_src elisp
  (advice-add #'package-activate :around
    (cl-function (lambda (fn &rest (&whole args package &optional force))
      (let* ((pkg-desc (cadr (assq package package-alist)))
             (version (when pkg-desc (package--incompatible-p pkg-desc :shallow))))
        (if (and version (not force))
            (message "Unable to activate package `%s'. Requires Emacs %s" package version)
          (apply fn args)))))
    '((name . dont-activate-packages-that-require-newer-emacs)))
#+end_src
...
[[https://melpa.org][MELPA]][[[elisp:(progn (unless (featurep 'cl-lib) (require 'cl-lib)) (unless (featurep 'package) (require 'package)) (customize-save-variable 'package-archives (let ((melpa (cl-assoc "melpa" package-archives :test #'equal)) (location "https://melpa.org/packages/")) (or (when melpa (setcdr melpa location) package-archives) (nconc package-archives '(("melpa" . location)))))))][добавить или заменить]]] и
...
Для максимальной простоты и надежности в условиях нестабильного
интернета лучше всего выбрать
[[https://www.mirrorservice.org/sites/melpa.org/packages/][официальное зеркало MELPA]][[[elisp:(progn (unless (featurep 'cl-lib) (require 'cl-lib)) (unless (featurep 'package) (require 'package)) (customize-save-variable 'package-archives (let ((melpa (cl-assoc "melpa" package-archives :test #'equal)) (location "https://www.mirrorservice.org/sites/melpa.org/packages/")) (or (when melpa (setcdr melpa location) package-archives) (nconc package-archives '(("melpa" . location)))))))][добавить или заменить]]]
([[https://github.com/melpa/melpa#mirrors][о зеркалах MELPA]]).

#+name: make-sure-melpa-is-used-as-one-of-community-package-archives
#+begin_src elisp
  (when (cl-loop for (id . location) in package-archives
                 never (equal "melpa" id)
                 never (string-match "https?://melpa.org/packages" location)
                 never (string-match "https?://www.mirrorservice.org/sites/melpa.org/packages" location))
    (nconc package-archives '(("melpa" . "https://www.mirrorservice.org/sites/melpa.org/packages/")))
    (customize-save-variable-after-init package-archives))
#+end_src

Система пакетов использует ключи GPG для проверки подлинности пакетов
загружаемых из архива GNU ELPA. Эти ключи имеют ограниченный срок
действия (например, первый ключ был действителен до сентября 2019
года). Устаревшие ключи вызывают ошибки проверки подписи пакетов и
делают архив GNU ELPA недоступным.

Согласно
[[https://elpa.gnu.org/packages/gnu-elpa-keyring-update.html][официальному рецепту]]
для обновления ключей на старых версиях эмакса и получения доступа к
GNU ELPA следует временно отключить проверку подписей пакетов и
установить пакет gnu-elpa-keyring-update.

#+name: make-sure-gnu-elpa-keyring-is-up-to-date-for-old-emacs
#+begin_src elisp
  (when (and (<= emacs-major-version 25)
             (not (package-installed-p 'gnu-elpa-keyring-update)))
...
кастомизируется стандартными средствами. Полное имя файла не
отображается в интерфейсе напрямую, узнать его можно только наведя
мышь на символ “-” перед именем буфера в моделайне. Так же при
использовании минибуфера его имя должно показываться после имени
буфера из которого он вызван, а не вместо этого имени. Если открыт
один единственный фрейм, заголовок по-умолчанию совершенно бесполезен,
поэтому используем наипростейший формат, устраняющий исходные
недостатки.

#+header: :noweb-ref customizations
#+begin_src elisp
  (setq-default frame-title-format
   '(:eval (let* ((minibuffer (when (minibufferp) (current-buffer))) 
                  (buffer (if (not minibuffer) (current-buffer)
             ...
             (with-current-buffer buffer
               (concat (when (file-remote-p default-directory)
                       ...
                       (or (w32-capitalize-filename-drive-letter
                             (or buffer-file-truename
                                 (when (stringp dired-directory) dired-directory)
                                 (when (consp dired-directory) (car dired-directory))))
                           (buffer-name))
                       (when minibuffer (format "%s" (buffer-name minibuffer))))))))
#+end_src

В интерфейсе Windows принято использовать
[[https://learn.microsoft.com/en-us/windows/win32/fileio/naming-a-file][большие буквы для имен дисков]].

#+header: :noweb-ref definitions
#+begin_src elisp
  (defun w32-capitalize-filename-drive-letter (filename)
    (seq-let [drive colon] filename
      (if (and (characterp drive) (<= ?a drive ?z) (eq colon ?:))
          (let ((new-filename (copy-sequence filename)))
            (aset new-filename 0 (upcase drive))
            new-filename)
        filename)))
#+end_src
...
Без этой настройки попытка сдвинуть курсор в пустом буфере вызывает
раздражительный громкий звон. Эмакс подает звуковой сигнал всякий раз
когда происходит отмена действия --- со стороны пользователя или из-за
невозможности его выполнения: например при прокрутке экрана вверх или
вниз у границы буфера, при перемещении курсора за пределы текстового
содержимого буфера, при нажатии неназначенных сочетаний клавиш, при
просмотре отсутствующей истории команд; при отмене поиска текста,
открытия файла, выполнения команды M-x и т. д. Некоторые режимы
воспроизведения звука (ASIO, WASAPI) блокируют звуковые сигналы
эмакса, делая звуковое предупреждение бесполезным.

[[elisp:(customize-group-other-window 'files)][Files]]⊲
По-умолчанию при изменении файла эмакс создает в месторасположении файла
[[https://www.gnu.org/software/emacs/manual/html_node/elisp/File-Locks.html][файл блокировки]] =.#file=,
при достаточно большом изменении ---
[[https://www.gnu.org/software/emacs/manual/html_node/elisp/Auto_002dSaving.html][файл автосохранения]] =#file#=,
а при сохранении ---
[[Https://www.gnu.org/software/emacs/manual/html_node/elisp/Backup-Files.html][резервную копию]] =file~=.

[[elisp:(customize-variable-other-window 'create-lockfiles)][Create Lockfiles]]
← [[elisp:(customize-save-variable 'create-lockfiles nil)][не создавать файлы блокировки одновременного редактирования файлов несколькими параллельно работающими эмаксами]]  ---
за ненадобностью на однопользовательских системах.
...
[[elisp:(customize-variable-other-window 'auto-save-file-name-transforms)][Auto Save File Name Transforms]]
← [[elisp:(customize-save-variable 'auto-save-file-name-transforms `((".*" ,(concat user-emacs-directory "auto-save/") t)))][автосохраненные версии файлов хранятся в директории ~/.emacs.d/auto-save/]].
Это необязательная настройка. В обычном режиме работы файлы
автосохранений автоудаляются и особо не мешают. Из-за простоты
реализации эта настройка системно-зависима; на разных системах
переменная
[[elisp:(describe-variable 'auto-save-file-name-transforms)][auto-save-file-name-transforms]]
имеет разное значение, по-умолчанию она указывает локальную системную
временную директорию для автосохранения не-локальных файлов.

[[elisp:(customize-group-other-window 'editing)][Editing]]⊲
[[elisp:(customize-group-other-window 'killing)][Killing]]⊲
По-умолчанию системный буфер обмена забивается /удаленными/
фрагментами текста. В основном удаление текста не предполагает его
отправку в буфер обмена. Чтобы слова, строки и регионы удаленные
командами
[[elisp:(describe-function 'kill-word)][kill-word]] (M-d, C-delete, C-backspace, M-DEL),
[[elisp:(describe-function 'kill-line)][kill-line]] (C-k) и
[[elisp:(describe-function 'kill-region)][kill-region]] (C-w, S-delete)
не отправлялись в буфер обмена можно посоветовать команде kill-region,
через которую они реализованы, считать что во время её выполнения
опция
[[elisp:(describe-variable 'select-enable-clipboard)][Select Enable Clipboard]]
отключена и взаимодействие с системным буфером обмена не происходит.

#+header: :noweb-ref customizations
#+begin_src elisp
  (defcustom select-enable-clipboard-for-kill-region t
    "Non-nil means killed text is copied to the clipboard. Option
  `select-enable-clipboard' must be set to non-nil."
    :type 'boolean
    :version "25.3.1"
    :group 'killing)

  (define-advice kill-region (:around (fn &rest args) selectively-enable-clipboard)
    (let ((select-enable-clipboard (and select-enable-clipboard
                                        select-enable-clipboard-for-kill-region)))
      (apply fn args)))
#+end_src

И тогда

[[elisp:(customize-variable-other-window 'select-enable-clipboard-for-kill-region)][Select Enable Clipboard For Kill Region]]
← [[elisp:(customize-save-variable 'select-enable-clipboard-for-kill-region nil)][удаленные фрагменты текста не попадают в буфер обмена операционной системы]].
В буфер обмена попадает текст сохраненный в «кольце удалений» командой
[[elisp:(describe-function 'kill-ring-save)][kill-ring-save]] (M-w, C-insert).

[[elisp:(customize-variable-other-window 'save-interprogram-paste-before-kill)][Save Interprogram Paste Before Kill]]
← [[elisp:(customize-save-variable 'save-interprogram-paste-before-kill t)][фрагмент текста из системного буфера обмена сохраняется в кольце удалений перед заменой удаленным в эмаксе фрагментом текста]].
Иначе при удалении слова, строки или региона скопированный вне эмакса
фрагмент текста находящийся в системном буфера обмена пропадет (но
останется в журнале буфера обмена Windows 10, 11, ... --- системном
«кольце удалений» [Windows+V]).
См. [[https://www.gnu.org/s/emacs/manual/html_node/emacs/Kill-Ring.html][Kill Ring]][[[info:Emacs#Kill Ring][инфо]]].

[[elisp:(customize-variable-other-window 'show-paren-mode)][Show Paren Mode]]
← [[elisp:(customize-save-variable 'show-paren-mode (customize-set-variable 'show-paren-mode t))][парные скобки подсвечиваются]].
Даже в небольших фрагментах лисп-кода на глаз невозможно понять к
какой открывающей скобке относится закрывающая. Закрывающую скобку
приходится удалять и ставить заново чтобы подсветить соответствующую
ей открывающую скобку.
...
Отступы табами старомодны и не подходят для лиспа в котором
используется специфическая неравномерная схема отступов --- она не
укладывается в логику табов и приводит к труднопредсказуемому
поведению курсора в области отступа. С табами невозможно указать
произвольное место для ручного выравнивания текста; без того чтобы
сперва не удалить их и не заменить пробелами.

Для написания символа табуляции используется сочетание
[[elisp:(describe-function 'quoted-insert)][C-q TAB]].

<<read-buffer-completion-ignore-case>>
[[elisp:(customize-variable-other-window 'read-buffer-completion-ignore-case)][Read Buffer Completion Ignore Case]]
← [[elisp:(customize-save-variable 'read-buffer-completion-ignore-case t)][автодополнение имени буфера не зависит от регистра]]
при переключении буферов по C-x C-b. Регистрозависимость требует
помнить несущественные детали и уделять много лишнего внимания при
поиске и наборе имени буфера во время переключения.

[[elisp:(customize-group-other-window 'editing)][Editing]]⊲
[[elisp:(customize-group-other-window 'matching)][Matching]]⊲
[[elisp:(customize-group-other-window 'bookmark)][Bookmark]]⊲
Самый обычный сценарий сразу после старта эмакса продолжить работу над
файлами которые были открыты в прошлый раз. Для этого предполагается
использовать
[[https://www.gnu.org/software/emacs/manual/html_node/emacs/Bookmarks.html][закладки]]
[[[info:Emacs#Bookmarks][инфо]]]
из меню Edit › Bookmarks --- создание закладки для местоположения
курсор​а в открытом файле C-x r m (/register mark/) помещает её в
список закладок C-x r l (/list/) после чего переход к закладке C-x r b
(/bookmark/) открывает файл с курсором на этом месте.
...
[[elisp:(customize-group-other-window 'environment)][Environment]]⊲
[[elisp:(customize-group-other-window 'minibuffer)][Minibuffer]]⊲
[[elisp:(customize-group-other-window 'savehist)][Savehist]]⊲
[[elisp:(customize-variable-other-window 'savehist-mode)][Savehist Mode]]
← [[elisp:(customize-save-variable 'savehist-mode t)][история команд минибуфера сохраняется]].
Каждая выполненная команда M-x сохраняется в
[[https://www.gnu.org/software/emacs/manual/html_node/emacs/Minibuffer-History.html][истории минибуфера]]
[[[info:Emacs#Minibuffer History][инфо]]].
Вспомнить или найти команду среди однажды выполненных команд
(с помощью
[[https://www.gnu.org/software/emacs/manual/html_node/emacs/Isearch-Minibuffer.html][инкрементального поиска в минибуфере]]
M-x C-r C-s ...
[[[info:Emacs#Isearch Minibuffer][инфо]]])
проще чем среди всех доступных команд. Для того чтобы список часто
используемых команд мог сформироваться история минибуфера должна
сохраняться.

[[elisp:(customize-group-other-window 'files)][Files]]⊲
[[elisp:(customize-group-other-window 'recentf)][Recentf]]⊲
Открытие файла с помощью перехода к закладке одновременно хорошо и
плохо сочетается с режимом Save Place потому что файл открывается на
указанном в закладке месте, а не на месте сохраненном в Save Place
(см. [[https://lists.gnu.org/r/emacs-devel/2020-06/msg00770.html][обсуждение в рассылке emacs-devel]]).
Несложно сделать создание закладок-без-сохранения-места, но есть режим
сохранения списка недавно открытых файлов, идеально сочетающийся с
режимом Save Place.

[[elisp:(customize-variable-other-window 'recentf-mode)][Recentf Mode]]
← [[elisp:(customize-save-variable 'recentf-mode t)][список недавно открытых файлов]].
Сочетание клавиш на усмотрение пользователя, традиционно C-x C-r
вместо открытия файла в режиме чтения которое никем не используется
(см. “[[https://www.masteringemacs.org/article/find-files-faster-recent-files-package][Find files faster with the recent files package]]”
в блоге [[https://www.masteringemacs.org][Mastering Emacs]])
или C-c r и C-x r e для единообразия с сочетаниями закладок.
...
#+begin_src elisp
  (with-eval-after-load 'recentf
    ...
    (define-key ctl-x-r-map (kbd "e") #'recentf-open-files))
#+end_src

[[elisp:(customize-group-other-window 'editing)][Editing]]⊲
[[elisp:(customize-group-other-window 'fill)][Fill]]⊲
[[elisp:(customize-variable-other-window 'sentence-end-double-space)][Sentence End Double Space]]
← [[elisp:(customize-save-variable 'sentence-end-double-space nil)][один пробел между предложениям]].
В эмаксе и во всей его документации принято
[[https://en.wikipedia.org/wiki/Sentence_spacing][соглашение эпохи пишущих машинок]]
что между предложениями должно быть два пробела. В современности такой
формат не используется нигде кроме как в текстовых файлах с
«[[https://jesseduffield.com/Hard-Wrap-vs-Soft-Wrap/][семантическими переносами]]»
(/semantic wrapping/, одно предложение на строку --- в основном для
того чтобы изменение одного предложения в тексте соответствовало
изменению одной строки в системе контроля версий).

Из-за этого для обычного текста
[[https://www.gnu.org/software/emacs/manual/html_node/emacs/Sentences.html][команды для предложений]]
M-a, M-e, M-k, C-x Backspace [[[info:Emacs#Sentences][инфо]]]
работают как [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Paragraphs.html][команды для абзацев]]
M-{, M-}, M-h [[[info:Emacs#Paragraphs][инфо]]],
что делает их бесполезными. Если отключить двойной пробел команды для
предложений будут частично работать, что лучше чем ничего.

[[elisp:(customize-group-other-window 'indent)][Indent]]⊲
[[elisp:(customize-variable-other-window 'tab-always-indent)][Tab Always Indent]]
← [[elisp:(customize-save-variable 'tab-always-indent 'complete)][автодополнение табом при установленных отступах]].
После однократного автодополнения (C-M-i) TAB временно переключается
на выполнение автодополнения как если бы эта опция была включена даже
если эта опция выключена.

[[elisp:(describe-variable 'completion-ignore-case)][Completion Ignore Case]]
← автодополнение не зависит от регистра для команд минибуфера M-x,
символов лиспа (C-M-i, Tab) и всего остального, кроме
[[read-buffer-completion-ignore-case][имен буферов]] и
[[elisp:(describe-variable 'read-file-name-completion-ignore-case)][имен файлов]].

Чувствительность к регистру заставляет обращать внимание на состояние
Caps Lock при наборе команд M-x и /заведомо/ знать регистр при поиске
имен /неизвестных/ функций и переменных C-h f/v. При этом в самом
Emacs есть только 3 набора команд которые чувствительны к регистру:
Buffer-menu-\ast{}, Custom-mode-\ast{} и Info-\ast{}, и никто, даже
[[https://lists.gnu.org/r/emacs-devel/2011-06/msg00454.html][создатели Emacs не знают зачем всё это нужно]].
...
[[elisp:(customize-group-other-window 'environment)][Environment]]⊲
[[elisp:(customize-group-other-window 'minibuffer)][Minibuffer]]⊲
[[elisp:(customize-variable-other-window 'completions-format)][Completions Format]]
← [[elisp:(customize-save-variable 'completions-format 'vertical)][подсказки автодополнения упорядочены по столбцам]],
а не по строкам. В алфавитно-упорядоченном по вертикали тексте легче
искать, если он умещается на одном экране. Оправдано когда подсказок
обычно мало. Прокрутка экрана с подсказками ↓ C-M-v C-M-S-v ↑.
...
Панель инструментов в эмаксе не доведена до ума. Единственная полезная
кнопка на панели инструментов это удаление буфера. Функции некоторых
кнопок непонятны даже с текстовыми подсказками. Во многих режимах
одинаковые иконки обозначают совершенно разные действия, что
увеличивает когнитивную нагрузку и замедляет реакцию.
...
если предполагается использовать эмакс только в терминале.
...
[[https://www.gnu.org/software/emacs/manual/html_node/elisp/Converting-to-Lexical-Binding.html][имена неиспользуемых аргументов должны начинаться с подчеркивания]].
...
#+begin_src elisp
  (let ((set-transparent-fringe
    ...
    (advice-add #'load-theme :after set-transparent-fringe '((name . set-transparent-fringe)))
    (advice-add #'disable-theme :after set-transparent-fringe '((name . set-transparent-fringe))))
#+end_src

Полоса прокрутки в минибуфере не выполняет никакую функцию, на одних
системах нарисована небрежно, на других не нарисована вообще. В Emacs
25 полоса прокрутки минибуфера наследуется от свойств фрейма и
периодически восстанавливается после отключения, поэтому скроллбар
приходится отключать во всех фреймах до и после захода в минибуфер,
после создания фрейма, выполнения команды и т. д.

#+header: :noweb-ref customize-well-tuned-emacs
#+begin_src elisp
  (defun hide-minibuffer-scroll-bar (&rest _)
    (dolist (frame (frame-list))
      (when (display-graphic-p frame)
        (let ((minibuf (minibuffer-window frame)))
          (when (window-live-p minibuf)
            (when (nth 2 (window-scroll-bars minibuf))
              (set-window-scroll-bars minibuf nil nil)))))))
#+end_src

Полоса прокрутки в минибуфере включается и выключается через отмену
или установку наследования свойств полосы прокрутки фрейма для всех
графических фреймов в которых есть живой минибуфер. Вертикальная
полоса прокрутки указана третьим параметром в списке (width columns
vertical-type height lines horizontal-type) возвращаемым
window-scroll-bars.

#+header: :noweb-ref customize-well-tuned-emacs
#+begin_src elisp
  (define-minor-mode hide-minibuffer-scroll-bar-mode
    "Hide the scroll bar in the minibuffer on each frame."
    :global t
    :group 'minibuffer
    :group 'frames
    (when hide-minibuffer-scroll-bar-mode
      (hide-minibuffer-scroll-bar)
      (force-window-update (minibuffer-window)))
    (let ((add-or-remove (if hide-minibuffer-scroll-bar-mode #'add-hook #'remove-hook)))
      (dolist (hook '(minibuffer-setup-hook minibuffer-exit-hook post-command-hook
                      after-make-frame-functions window-configuration-change-hook))
        (funcall add-or-remove hook #'hide-minibuffer-scroll-bar)))
    (unless hide-minibuffer-scroll-bar-mode
      (dolist (frame (frame-list))
        (let ((minibuf (minibuffer-window frame)))
          (when (window-live-p minibuf)
            (set-window-scroll-bars minibuf nil t))))))
#+end_src

[[https://www.gnu.org/software/emacs/manual/html_node/elisp/Defining-Minor-Modes.html][Объявление глобального режима создает переменную в системе настроек Customize]]
[[[info:ELisp#Defining Minor Modes][инфо]]]:

[[elisp:(customize-variable-other-window 'hide-minibuffer-scroll-bar-mode)][Hide Minibuffer Scroll Bar Mode]]
← [[elisp:(customize-save-variable 'hide-minibuffer-scroll-bar-mode t)][полоса прокрутки в минибуфере скрыта]].
...
[[elisp:(customize-group-other-window 'files)][Files]]⊲
[[elisp:(customize-group-other-window 'uniquify)][Uniquify]]⊲
...
без угловых скобок
(см. [[elisp:(describe-variable 'uniquify-buffer-name-style)][Uniquify Buffer Name Style]], [[info:Emacs#Uniquify][инфо]]).
Стандартные угловые скобки вокруг файлового пути создают визуальный шум,
делают элементы менее отчетливыми.
...
вместо стартового экрана (C-h C-a), или как вариант ---
...
#+begin_src elisp
  (use-package smartparens-config :ensure smartparens
    ...
    :init
      <<backport-fixnump-from-emacs-27>>
    :config
      (progn
  ...
  (advice-add #'backward-kill-word-or-kill-region :around
    (lambda (fn &rest args)
      (if (when smartparens-mode (not (region-active-p)))
        ...
        (apply fn args)))
    '((name . backward-kill-word-preserve-s-expression-structure)))
#+end_src
...
Для проверки «исходников» текста есть режим [[elisp:(describe-function 'visible-mode)][visible-mode]]
который делает невидимый текст видимым, без отключения основного
режима. Очень удобно для редактирования этого руководства.

#+header: :noweb-ref keybindings
#+begin_src elisp
  (with-eval-after-load 'org
    (define-key org-mode-map [f2] #'visible-mode))
#+end_src
...
Удаление буфера очень частая операция, но подтверждение сильно
замедляет и отвлекает внимание ради возможности удаления /другого/
буфера, чего не происходит практически никогда (на замену удаленному
видимому придет произвольный буфер, а невидимые удаляются из
list-buffers, ibuffer и т. д.).
...
Для перехода по истории страниц в режимах Help и Info вместо f
(/forward/) и b (/backward/) как во всех обычных режимах
редактирования, используется l (/left?/, исходно /last/) и r
(/right?/, /return?/, появилась в Emacs 22) которые не используются ни
в каких других режимах редактирования. Это интерфейс с большой
когнитивной нагрузкой. Когда кто-то хочет вернуться назад в истории он
думает: «Так, мне нужно назад. Назад это как стрелка в браузере.
Стрелка показывает в какую сторону? Влево или вправо. Лево и право это
где?» В браузере кнопки ← и → не воспринимаются как кнопки
пространственного направления влево и вправо, не привязываются к
словам лево и право. Здесь нужно вспомнить как называется каждое из
совершенно симметричных направлений для привязки к несимметричным во
времени операциям. «Лево это там, значит кнопка l». Где на клавиатуре
кнопка l? Справа от кнопки r.

В [[https://cgit.git.savannah.gnu.org/cgit/emacs.git/commit/?id=dc25712992115aa778954660544e366b6df35f5b][коммите 2005-го года]]
в Info добавили команду forward, назначив её на r (для несовместимости
эмакса с собственными конвенциями), и переименовали команду last в
backward оставив её на l (для обратной совместимости непонятно с чем).

В [[https://cgit.git.savannah.gnu.org/cgit/emacs.git/commit/?id=b216f0a6cf524023eefbedcff3a70c8763e7e3e0][коммите 2013-го года]]
l и r стали частью Help для совместимости с Info, сочетания клавиш
которого несовместимы с общим дизайном эмакса ради обратной
совместимости с режимом на уровне дизайна не предполагавшем наличие
таких функций.

В [[https://cgit.git.savannah.gnu.org/cgit/emacs.git/commit/?id=924d6997866a444f833ea0a2f0da25058a351564][коммите 2013-го года]]
в браузер EWW добавили команду back, назначив её на l. (EWW стал
частью Emacs неделей раньше).

В [[https://cgit.git.savannah.gnu.org/cgit/emacs.git/commit/?id=d3f0f918e9f2f20d27e129107a63e634d4e82814][коммите 2013-го года]]
в EWW добавили команду forward, назначив её на f.

В [[https://cgit.git.savannah.gnu.org/cgit/emacs.git/commit/?id=7a409b30053cf0c48ff5de7c5d9b408493df1054][коммите 2013-го года]]
после [[https://debbugs.gnu.org/cgi/bugreport.cgi?bug=16178#10][баг-репорта]]
l и r стали частью EWW для совместимости с Info. Через пол года после
предыдущего коммита.

Это показательный пример ползучего «эволюционного дизайна» который
приводит к рассогласованию и усложнению интерфейса без причины.
...
#+begin_src elisp
  ...
  (with-eval-after-load 'info
    ...
    (define-key Info-mode-map (kbd "r") #'Info-follow-reference))
  (with-eval-after-load 'eww
    (define-key eww-mode-map (kbd "f") #'eww-forward-url)
    (define-key eww-mode-map (kbd "b") #'eww-back-url)
    (define-key eww-mode-map (kbd "m") #'eww-add-bookmark))
#+end_src
...
_[[elisp:(describe-function 'sp--syntax-class-to-char)][sp--syntax-class-to-char]]: Symbol’s function definition is void: fixnump_

Smartparens использует функцию [[elisp:(describe-function 'fixnump)][fixnump]]
[см. [[https://emacsdocs.org/docs/elisp/Predicates-on-Numbers#function-fixnump-object][Emacs Docs]]]
из Emacs 27 в обход зависимостей из-за чего удаление слов не происходит.

#+name: backport-fixnump-from-emacs-27
#+begin_src elisp
...
_Emacs 25: фризы при движении курсора в окне с Unicode-символами в Windows_

Очень сильные фризы или даже зависание при движении курсора в окне с
символами для показа которых Emacs переключается на другие шрифты
(иероглифы, эмодзи, математические символы, индексы) из-за
[[https://lists.gnu.org/archive/html/bug-gnu-emacs/2016-11/msg00469.html][неправильного управления кешем шрифтов]],
см. [[elisp:(describe-variable 'inhibit-compacting-font-caches)][inhibit-compacting-font-caches]].
