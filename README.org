    ...
    (defun package-install-selected-packages ()
      ...
      (let ((packages-to-be-installed (cl-remove-if #'package-installed-p package-selected-packages)))
        (when packages-to-be-installed
          (when (y-or-n-p (format "%s packages will be installed:\n%s, proceed?"
...
нестандартные сочетания клавиш и ) придется написать несколько строк на
    ...
    (defun well-tuned-emacs-scratch-buffer-message ()
      (with-current-buffer "*scratch*"
        (let ((scratch-buf-last-char (point-max)))
          ...
          (let ((password-vault "~/Dropbox/Passwords.org.gpg"))
            (when (file-exists-p password-vault)
              (fancy-splash-insert
               " "
               :link (list "Passwords" (lambda (_button) (find-file password-vault))
...
[[http://raskin-interface.narod.ru/interface/chapter5.htm#s5.5][классический курсор предпочтительнее]], чем
современная [[elisp:(customize-save-variable 'cursor-type 'bar)][вертикальная черта между букв]].
Однако классический курсор не идеален и было бы неплохо делать его
[[elisp:(customize-save-variable 'cursor-type
 'hollow)][прозрачным]] во время выделения текста (TBD).
...
в них введенные спецслова превращаются в спецсимволы, например 
\​'e → é, \​th → þ, \​Mu\​epsilon\​nu → Μεν,
  ...
  (defcustom well-tuned-emacs-fonts
    (quote
     (((gnu/linux gnu/kfreebsd darwin windows-nt cygwin)
       "Consolas-10" "Courier New-9" "Monaco-9" "DejaVu Sans Mono-10"
       "Cambria-11" "Segoe UI Symbol-12" "Lucida Sans Unicode-10" 
       "Menlo-10" "Anonymous Pro-11" "Envy Code R-10")))
...
#+begin_src elisp
  (defun well-tuned-emacs-fonts ()
    (cl-rest (cl-assoc system-type well-tuned-emacs-fonts :test #'member)))
#+end_src

_available-font_ → полное имя шрифта, если шрифт с указанным кратким
именем доступен в текущем системном окружении.

#+name: customize-well-tuned-emacs
#+begin_src elisp
  (defun available-font (font)
    (when (stringp font) (find-font (font-spec :name font))))
#+end_src
...
подробнее на странице 110. Можно считать это просто рекомендацией
...
языковых средств которые впоследствии станут мейнстримом только через
...
_Альтернативный вариант_: [[elisp:(customize-variable-other-window 'electric-pair-mode)][Electric Pair Mode]]
← [[elisp:(customize-save-variable 'electric-pair-mode t)][скобки вводятся парами]].
Открывающая скобка автоматически дополняется закрывающей.

_Альтернативный вариант_: [[elisp:(customize-variable-other-window 'show-paren-mode)][Show Paren Mode]]
← [[elisp:(customize-save-variable 'show-paren-mode t)][парные скобки подсвечиваются]].

Для эмакса написано много лисп-программ делающих работу со скобками
более удобной и наглядной. Режимы Electric Pair и Show Paren --- это
встроенные лисп-программы, они достаточно хороши, но есть и получше,
например [[https://github.com/Fuco1/smartparens][Smartparens]] и Paredit.
...
#+begin_src elisp
  (use-package smartparens-config :ensure smartparens
    ...
    :config
      (progn
        <<bind-paredit-like-keys-for-smartparens-mode>>
        <<turn-on-smartparens-strict-mode-for-some-modes>>
        (smartparens-global-mode (cl-first (get 'smartparens-global-mode 'saved-value)))
        (show-smartparens-global-mode (cl-first (get 'show-smartparens-global-mode 'saved-value)))))
#+end_src
...
#+name: turn-on-smartparens-strict-mode-for-some-modes
#+begin_src elisp
...
См. [[http://danmidwood.com/content/2014/11/21/animated-paredit.html][анимированное руководство по сочетаниям клавиш в лисп-программе Paredit]].

#+name: bind-paredit-like-keys-for-smartparens-mode
#+begin_src elisp
...
← [[elisp:(customize-save-variable 'superword-mode t)][составные слова набранные в любом стиле считаются единым словом]].
...
а не на позиции курсора мыши. В традиции X Window System текст
вставляется по щелчку средней кнопки мыши.

[[https://en.wikipedia.org/wiki/X_Window_System][X Window System]] и
Emacs (в любой ОС) поддерживают два буфера обмена: /primary/ и
/clipboard/. Мышь работает с primary-буфером, клавиатурные команды ---
с clipboard-буфером.
...
в буфере скопированных/удаленных фрагментов текста /kill ring/.

[[elisp:(customize-variable-other-window 'apropos-do-all)][Apropos Do All]]
← [[elisp:(customize-save-variable 'apropos-do-all t)][расширенный поиск командами apropos]].
...
Сочетания каксе имеют три ярко выраженных особенности: их
...
#+begin_src elisp
  (use-package guide-key
    ...
    :init (guide-key-mode (cl-first (get 'guide-key-mode 'saved-value))))
#+end_src
...
для [[elisp:(customize-variable-other-window 'guide-key/guide-key-sequence)][некоторых префиксов]],
например:
C-x [[[elisp:(customize-save-variable 'guide-key/guide-key-sequence (cl-pushnew "C-x" guide-key/guide-key-sequence :test #'equal))][добавить]]],
...
C-x 4 [[[elisp:(customize-save-variable 'guide-key/guide-key-sequence (cl-pushnew "C-x 4" guide-key/guide-key-sequence :test #'equal))][добавить]]],
C-x 5 [[[elisp:(customize-save-variable 'guide-key/guide-key-sequence (cl-pushnew "C-x 5" guide-key/guide-key-sequence :test #'equal))][добавить]]],
C-x 8 [[[elisp:(progn (dolist (k '("C-x 8" "C-x 8 a")) (cl-pushnew k guide-key/guide-key-sequence :test #'equal))(customize-save-variable 'guide-key/guide-key-sequence guide-key/guide-key-sequence))][добавить]]].
...
или [[elisp:(customize-save-variable 'guide-key/popup-windows-position 'right)][справа]].
...
Лисп-программа ibuffer это улучшенная версия программы list-buffers.
...
Проще убить текущий буфер не отпуская клавишу Control.
...
Автодополнение текста выполняется командой hippie-expand вместо dabbrev-expand.
...
← [[elisp:(customize-save-variable 'hippie-expand-try-functions-list '(try-expand-dabbrev try-expand-dabbrev-all-buffers try-expand-dabbrev-from-kill try-complete-file-name-partially try-complete-file-name try-expand-all-abbrevs try-expand-list try-expand-line try-complete-lisp-symbol-partially try-complete-lisp-symbol))][мощная, но умеренная последовательность вариантов автодополнений]].
Оригинальная последовательность автодополнений иногда выдает варианты
с небывалым размахом.

----------------------------------------------------------------------

В [[http://www.gnu.org/software/emacs/manual/html_node/emacs/Windows-HOME.html][Windows Vista/7/8/10]]
...
#+begin_src elisp
  (when (= emacs-major-version 25)
    (declare-function font-lock-ensure "font-lock" ())
