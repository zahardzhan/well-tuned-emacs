...
частности. Лиспа не нужно бояться, даже не смотря на то что это
древнейший из ныне используемых языков программирования, а программы
на нем выглядяет совершенно необычно. Суть лиспа заключается в том,
что это наипростейший из известных человечеству способов организации
вычислений из /слов/, /структур данных/ и /функций/ (открытых
математиками в 1936 году).
...
[[elisp:list-packages][открыть её графический интерфейс в эмаксе]] --- [[elisp:list-packages][M-x list-packages]]]
...
Исходный код лисп-программы инициализации
[[elisp:(find-file actual-user-init-file)][в файле actual-user-init-file]]
...
определенном месте, по умолчанию --- в
[[elisp:(dired (file-name-directory actual-user-init-file))][той же директории]],
что и актуальная лисп-программа инициализации эмакса; под определенным
именем, по-умолчанию ---
...
[[[elisp:(customize-save-variable 'custom-file "~/.emacs.d/custom/custom.el")][сохранить на будущее как значение это опции]]].
...
[[https://melpa.org][MELPA]][[[elisp:(progn (unless (featurep 'cl-lib) (require 'cl-lib)) (unless (featurep 'package) (require 'package))(cl-pushnew (cons "melpa" "http://melpa.milkbox.net/packages/") package-archives :test #'equal))][использовать этот архив]]]
и
[[https://marmalade-repo.org/][Marmalade]][[[elisp:(progn (unless (featurep 'cl-lib) (require 'cl-lib)) (unless (featurep 'package) (require 'package)) (cl-pushnew (cons "marmalade" "http://marmalade-repo.org/packages/") package-archives :test #'equal))][использовать и этот архив тоже]]]
