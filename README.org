...
дел пугает, но все не так плохо: эмакс сложен для сложных задач и
прост для простых, все зависит только от ваших потребностей. Начать
знакомство с ним лучше с общего обзора
...
[[file:~/.emacs][.emacs]] или [[file:~/.emacs.d/init.el][init.el]] (/сплести/ программу --- /tangle/)
...
Многие пакеты слабо взаимодействуют с собственными сохраненными
настройками, часто придется загружать их вручную.

#+name: customize-customize-and-apply-customizations
#+begin_src elisp
  (defun saved-value (var)
    (cl-first (get var 'saved-value)))
#+end_src
...
получше, например [[https://github.com/Fuco1/smartparens][Smartparens]] и Paredit.
  ...
  (use-package smartparens-config :ensure smartparens
    ...
    :config
      (progn
        ...
        (smartparens-global-mode (saved-value 'smartparens-global-mode))
        (show-smartparens-global-mode (saved-value 'show-smartparens-global-mode))))
#+end_src
...
#+begin_src elisp
  (use-package guide-key
    ...
    :init (guide-key-mode (saved-value 'guide-key-mode)))
#+end_src
...
Не отпуская клавишу Control можно легко убить много буферов подряд.
