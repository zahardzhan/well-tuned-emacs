...
Во время запуска эмакс выполняет [[https://github.com/emacs-mirror/emacs/blob/master/lisp/startup.el#L1158][одну из девяти возможных программ
инициализации]][[[elisp:(describe-function 'command-line)][исходник]]] (на самом деле их больше, но другие варианты
не кроссплатформенны), и нам нужно определить какую именно исходную
...
|---------------------------+-------------------------------------------+------------------------------------|
| nil                       | nil                                       | ~/.emacs.d/init.el or ~/.emacs !!! |
...
#+begin_src elisp -r -n
  ...
  (init.el (file-truename "~/.emacs.d/init.el"))
  (init.elc (file-truename "~/.emacs.d/init.elc"))
#+end_src
      ...
      (or (when (equal user-init-file nil)
          ...
          (when (file-equal-p user-init-file .emacs.elc)
            (or (when (file-exists-p .emacs.el)
                  .emacs.el)
                .emacs))
          (when (or (file-equal-p user-init-file init.el)
                    (file-equal-p user-init-file init.elc))
            init.el))))
#+end_src
...
Reference File]] ← файл "README.org" в директории с актуальной
  ...
  (defcustom well-tuned-emacs-reference-file
    (let* (
      ...
      (or (when (or (file-equal-p actual-user-init-file .emacs)
                    ...
                    (file-equal-p actual-user-init-file _emacs)
                    (file-equal-p actual-user-init-file _emacs.el))
...
[[elisp:(customize-group-other-window 'emacs)][Emacs]]⊲[[elisp:(customize-group-other-window%20'help)][Help]]⊲[[elisp:(customize-group-other-window%20'customize)][Customize]]⊲[[elisp:(customize-variable-other-window 'custom-file)][Custom File]] ← [[elisp:(concat (file-name-as-directory (concat user-emacs-directory "custom")) "custom.el")][
...
#+begin_src elisp -r -n
  (progn
    ...
    (if well-tuned-emacs-compile-user-init-file
        (progn (byte-compile-file actual-user-init-file 'load)
